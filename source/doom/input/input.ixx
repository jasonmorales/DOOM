...
export struct event
{
	...
	constexpr bool is(event_id test_id) const { return test_id == event_id::Any || test_id == id; }
	constexpr bool down(event_id test_id = event_id::Any) const { return flags.all(event_flag::down) && is(test_id); }
	constexpr bool up(event_id test_id = event_id::Any) const { return flags.all(event_flag::up) && is(test_id); }
	constexpr bool ctrl() const { return flags.all(event_flag::ctrl); }
	constexpr bool shift() const { return flags.all(event_flag::shift); }
	constexpr bool alt() const { return flags.all(event_flag::alt); }
	constexpr bool changed(event_id test_id = event_id::Any) const { return flags.all(event_flag::changed) && is(test_id); }
	constexpr bool pressed(event_id test_id = event_id::Any) const { return down(test_id) && (changed() || repeated()); }
	constexpr bool released(event_id test_id = event_id::Any) const { return up(test_id) && changed(); }
	constexpr bool repeated(event_id test_id = event_id::Any) const { return flags.all(event_flag::repeated) && is(test_id); }
	constexpr bool held(event_id test_id = event_id::Any) const { return flags.all(event_flag::held) && is(test_id); }
	...
	{
		//return std::format("{{{}|{}: {}x ({}, {}) [{}] {}}}", device.name(), id.name(), count, cursor_pos.x, cursor_pos.y, flags.str(), i_value);
		return "";
	}
};
