...
void P_LineOpening(line_t* _linedef)
{
    sector_t* front;
    sector_t* back;

    if (_linedef->sidenum[1] == -1)
    {
        // single sided line
        openrange = 0;
        return;
    ...
    front = _linedef->frontsector;
    back = _linedef->backsector;
    ...
    fixed_t	maxfrac)
{
    ...
    intptr_t count = intercept_p - intercepts;

    in = 0;			// shut up compiler warning

    while (count--)
    {
        dist = std::numeric_limits<fixed_t>::max();
        ...
        in->frac = std::numeric_limits<decltype(in->frac)>::max();
    }
